...
const FreqOfPaymentConfig = ({
  ...
  const handleEditClick = () => {
    ...
    setEditedFreqOfPayment(
      configExists(employerInfo, "Frequency of Payment")
        ? employerInfo.find(
  ...
  const handleSaveClick = async () => {
    ...
    if (response.status === 200) {
      ...
      const updatedEmployerInfo = employerInfo.map((config) => {
      ...
      setEmployerInfo(updatedEmployerInfo);
    } else {
      toast.current.show({
        severity: "error",
        summary: "Error",
...
export default FreqOfPaymentConfig;
